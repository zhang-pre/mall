...
public class CategoryServiceImpl extends ServiceImpl<CategoryDao, CategoryEntity> implements CategoryService {
    ...
    //递归查找所有菜单的子菜单
    //note 注意结束递归的条件是什么，filter是递归结束的条件
    private List<CategoryEntity> getChildren(CategoryEntity root,List<CategoryEntity> alls) {
        List<CategoryEntity> children =
                alls.stream()
                        .filter(categoryEntity -> categoryEntity.getParentCid().equals(root.getCatId()))
                .map(menu -> {
                    menu.setChildren(getChildren(menu, alls));
                    return menu;
                }).sorted(
                        (menu1, menu2) -> {
